...
class SimpleWtGym(gym.Env):
    ...
    def step(self, action):
        ...
        if done==True and self.wt_sim.ti < self.t_max:
            #If it fails, add the maximum reward it would have gotten, getting -1 in every step
            lost_rewards = (self.t_max-self.wt_sim.ti)/self.wt_sim.dt
    ...
    def do_terminate(self):
        ...
        if (self.wt_sim.ti >= self.t_max):
            ...
            terminate = True    
        elif  (self.wt_sim.wt.w < 0.2):
            logging.debug("Terminating episode. Generator stopped")
            terminate = True
